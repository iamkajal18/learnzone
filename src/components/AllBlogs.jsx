  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const response = await axios.delete(`/api/delete-blog/${id}`, {
        headers: { Authorization: `Bearer ${session?.user?.token}` }, // Add authentication if needed
      });
      if (response.data.success) {
  ...
  const LoadingSkeleton = () => (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {[...Array(6)].map((_, i) => (
        <div
          key={i}
          className="animate-pulse bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-100 dark:border-gray-700"
        >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold">
            <span className="text-teal-500 dark:text-teal-400">Creative Blogs</span>{" "}
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm text-gray-600 dark:text-gray-300">
                      {idea.authorEmail ? stripHtml(idea.authorEmail.split("@")[0]) : "Anonymous"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    {session?.user?.email === idea.authorEmail && (
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          aria-disabled={deletingId === idea._id}
                        >
                          {deletingId === idea._id ? (
